...
class HttpServer:
    ...
    def process_new_request(self, request):
        ...
        # manually unpack the url as needed.
        url = raw_path
        if '#' in url:
    ...
    def dispatch(self, request):
        ...
        if not func:
            func = self.dispatch_route(request)
        if not func:
    ...
    def dispatch_route(self, request):
        try:
            ...
            p = request.path[1:]
            #async.debugbreak()
            prefix = r.longest_prefix(p)
            #prefix = self.routes.longest_prefix(request.path[1:])
            if prefix and hasattr(self, prefix):
    ...
    def sendfile(self, request, path):
        ...
        st = os.stat(path=path, follow_symlinks=False)
        size = st[6]
